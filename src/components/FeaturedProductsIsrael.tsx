...
export const FeaturedProductsIsrael = () => {
  ...
  return (
    <section dir="rtl" className="py-12 px-5 bg-background mb-12">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {featuredProducts.map((product, index) => (
            <div
              key={index}
              className="bg-card border-2 border-border rounded-2xl p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-blue-500 flex flex-col"
            >
              {/* Icon */}
              <div className="text-5xl text-center mb-4">{product.icon}</div>

              {/* Product Name */}
              <h3 className="text-lg font-semibold text-card-foreground text-center mb-4 leading-tight">
                {product.name}
              </h3>
              ...
              <Button
                asChild
                className="w-full bg-blue-600 hover:bg-blue-700 hover:scale-[1.02] transition-all text-white py-3 text-base font-semibold rounded-lg"
              >
                <a
                  href={product.link}
                  target="_blank"
